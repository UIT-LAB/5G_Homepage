...
router.get('/notice/:num', function(req, res, next) {
  db.query('select * from Notice_Board ORDER BY n_writer_date DESC ', function (error, result) {
    if (error) {
      ...
      res.render('board/notice', {result : result, n_num : req.params.num, max_value:15, dayjs ,id:req.session.u_id,name:req.session.u_name });
    };
  });
});
    ...
    if (error) {
      ...
      db.query(`update Notice_Board set n_view = n_view+1 where nid = ?`,req.params.num , function (error, n_view) {
        if(error){
          ...
          result[0].n_view++;
          res.render('board/notice_detail', {result : result, n_num : req.params.num, max_value:15, dayjs, id:req.session.u_id,name : req.session.u_name});
        }
        else {
          res.render('error');
        } 
...
var sql = {n_title:title, n_content : content, n_writer:req.session.u_name, n_writer_date : datetime, n_view : 0};
    ...
    if (error) {
      ...
      db.query(`update Post_Board set p_view = p_view+1 where pid = ?`,req.params.num , function (error, n_view) {
        if(error){
          ...
          result[0].p_view++;
          res.render('board/post_detail', {result : result, p_num : req.params.num, max_value:15, dayjs, id:req.session.u_id,name : req.session.u_name});
        }
        else {
          res.render('error');
        } 
...
router.get('/question/detail/:num', function(req, res, next) {
  db.query('select * from Question_Board where qid = ?', req.params.num, function (error, result) {
      if (error) {
        ...
        res.render('board/question_detail', {result : result, q_num : req.params.num, max_value:15, dayjs, name:req.session.u_name, id: req.session.u_id  });
      }
      else {
        res.render('error');
      }
...
var datetime = date.format('YYYY-MM-DD HH:mm:ss');
var sql = {q_title:title, q_content : content, user_id : req.session.u_name, write_date : datetime};
